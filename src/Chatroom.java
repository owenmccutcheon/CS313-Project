...
public class Chatroom {
    ...
    private static final Map<String, List<String>> mailboxes = new HashMap<>();
    private static final Set<ClientHandling> clients = Collections.synchronizedSet(new HashSet<>());
    private static final Map<String, ClientHandling> usernames = new ConcurrentHashMap<>();

    // THREAD SAFE GROUP STORAGE
    private static final Map<String, Set<ClientHandling>> groups = new ConcurrentHashMap<>();
    ...
    public static void sendMail(String recipient, String message, ClientHandling sender) {
        ...
        List<String> list = mailboxes.get(recipient);

        if (list == null) {
            try {
                Thread.sleep(50); // force race condition
            } catch (InterruptedException ignored) {}

            list = new ArrayList<>();
            mailboxes.put(recipient, list);
        }
    ...
    public static List<String> getMail(String username) {
        List<String> messages = mailboxes.get(username);
